...
class PDFLearningAssistant:
    ...
    def __init__(self, user_id: str = "default_user"):
        ...
        """
        self.user_id = user_id
        self.session_id = f"session_{datetime.now().strftime('%Y%m%d%H%M%S%f')}"
...
"""
几个关键的设计决策：
    MemoryTool的初始化：通过user_id参数实现用户级别的记忆隔离。
    RAGTool的初始化：通过rag_namespace参数实现知识库的命名空间隔离
...
def load_document(self, pdf_path: str) -> Dict[str, Any]:
    ...
    if not os.path.exists(pdf_path):
        return {"success": False, "message": "PDF文件不存在：{pdf_path}"}
    ...
    if result.get("success", False):
        ...
        self.memory_tool.execute(
            "add",
            content=f"加载文档: {self.current_document}",
            memory_type="episodic",
            importance=0.9,
        ...
        return {
            "success": True,
            "message": f"加载成功！(耗时：{process_time:.2f}秒)",
...
def ask(self, question: str, use_advanced_search: bool = True) -> str:
    ...
    Args：
        question: 提问的问题
        use_advanced_search: 是否使用高级搜索(MQE+HyDE)
    Returns:
        str: 提问的答案
    """
...
![Hello-agents第八章知识总结.png](../images/Hello-agents第八章知识总结.png)