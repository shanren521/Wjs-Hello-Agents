...
from hello_agents.memory import MemoryConfig, MemoryManager, MemoryItem
...
class NPCAgentManager:
    ...
    def _create_agents(self):
        ...
        for name, role in NPC_ROLES.items():
            try:
                ...
                print(f"创建NPC {name} Agent创建失败：{e}")
    ...
    def _create_memory_manager(self, npc_name: str) -> MemoryManager:
        ...
        memory_config = MemoryConfig(
            ...
            importance_threshold=0.3  # 重要性低于0.3的记忆会被遗忘
        )
        ...
        memory_manager = MemoryManager(
            ...
            enable_working=True,  # 开启工作记忆(短期)
            enable_episodic=True,  # 开启情景记忆(长期)
...
"""
            ...
            print(f"{npc_name}对话失败：")
            import traceback
            traceback.print_exc()
            return f"抱歉，我现在有点忙，等会再聊吧。(错误：{str(e)})"
