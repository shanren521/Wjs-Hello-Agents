...
+ 重复：重复第2步，直到词表大小达到预设的阈值。

案例演示：假设我们的迷你语料库是 {"hug": 1, "pug": 1, "pun": 1, "bun": 1}，并且我们想构建一个大小为 10 的词表。BPE 的训练过程可以用下面的表：

![BPE 算法合并过程示例.png](../../images/BPE%20算法合并过程示例.png)
...
+ WordPiece: Google BERT模型采用的算法，与BPE非常相似，但合并词元的标准不是最高频率，而是能最大化提升语料库的语言模型概率，会优先合并哪些能让整个语料库的通顺度提升最大的词元对。
+ SentencePiece：Google开源的一款分词工具，Llama系列模型采用了此算法。最大的特点是，将空格也视作一个普通字符。使得分词和解码过程完全可逆，且不依赖于特定的语言。

#### 3.2.2.3 分词器对开发者的意义

+ 上下文窗口限制：模型的上下文窗口是以Token数量计算的，而不是字符数或单词数。
+ 模型表现的异常：有时模型的奇怪表现根源在于分词。例如，模型可能很擅长计算2 + 2，但对于2+2(没有空格)就可能出错。

### 3.2.3 调用开源大语言模型

在transformers库中，通常使用AutoModelForCausalLM和AutoTokenizer来自动加载于模型匹配的权重和分词器
...
# 使用模型生成回答
# max_new_tokens控制了模型最多能生成多少个新的Token
...
+ 性能于能力：最核心的考量。
+ 成本：API 费用，本地部署硬件支持
+ 速度(延迟)：轻量化或优化过的模型，实时交互的智能体(客服、游戏NPC)
+ 上下文窗口：模型能一次性处理的Token数量上限。
+ 部署方式：API 受服务商的限制。本地部署自主可控，技术和硬件要求高
+ 生态与工具链：主流模型通常拥有更丰富的社区支持、教程、预训练模型、微调工具和兼容的开发框架(如LangChain，LlamaIndex)
+ 可微调性与定制化：允许开发者使用自己的数据集对模型进行定制化训练。
+ 安全性与伦理：选择模型时，要考虑模型在偏见、毒性、幻觉等方面的表现。
...
+ Grok 系列
+ Anthropic Claude系列
+ 国内主流模型：文心一言、混元、讯飞星火、月之暗面、Qwen、豆包
...
揭示了模型性能与模型参数量、训练数据量以及计算资源之间存在着可预测的幂律关系

在对数-对数坐标系下，模型的性能(损失Loss来衡量)与参数量、数据量和计算量这三个因素都呈现出平滑的幂律关系，在资源允许的范围内，尽可能地扩大模型规模和训练数据量。

模型参数量和训练数据量之间存在一个最优配比。

当模型规模达到一定阈值后，会突然展现出在小规模模型中完全不存在或表示不佳的全新能力。例如：链式思考、指令遵循、多步推理、代码生成，都是在参数量达到百亿或千亿后出现的

### 3.3.2 模型幻觉

本质是模型在生成过程中，过度自信地编造了信息，而非准确地检索或推理。
+ 事实性幻觉：模型生成与现实世界不符的信息。
+ 忠实性幻觉：在文本摘要、翻译等任务中，生成的内容未能忠实地反映源文本的含义。
+ 内在幻觉：模型生成的内容与输入信息直接矛盾。

缓解幻觉的方法：
+ 数据层面：高质量数据清洗、引入事实性知识以及强化学习与人类反馈等方式，从源头减少幻觉。
+ 模型层面：探索新的模型架构，或让模型能够表达其对生成内容的不确定性。
+ 推理与生成层面：
  + 检索增强生成RAG：通过在生成之前从外部知识库(文档数据库、网页)中检索相关信息，然后将检索到的信息作为上下文，引导模型生成基于事实的回答。
  + 多步推理与验证：引导模型进行多步推理，并在每一步进行自我检查或外部验证。
  + 引入外部工具：允许模型调用外部工具来获取实时信息或进行精确计算。
...
+ N-gram模型的核心假设是马尔可夫假设。请解释这个假设的含义，以及N-gram模型存在哪些根本性局限？
  + 答案：当前词只依赖前N-1个词，与更早的历史无关(每一步都依赖前一步，无法并行！)。优点：大幅降低计算复杂度，减少参数量。缺点：丢失长距离依赖信息。
         局限：数据稀疏性、长距离依赖缺失、无语义理解、泛化能力差、参数爆炸(N增大时，参数量指数增长)
+ 神经网络语言模型（RNN/LSTM）和Transformer分别是如何克服N-gram模型局限的？它们各自的优势是什么？
  + RNN/LSTM核心机制：隐藏状态，优势：参数共享、词嵌入、连续表示。局限：梯度消失、串行处理，训练慢
  + Transformer核心：自注意力(让序列中的每个词都能"看到"并"关注"其他所有词，动态计算它们之间的相关性。每个位置的计算相互独立，只需要矩阵乘法)。
                    优势：全局注意力、并行计算、多头注意力、位置编码、深层堆叠。
...
+ 自注意力机制（Self-Attention）的核心思想是什么？
  + 答案：让序列中的每个词都能"看到"并"关注"其他所有词，动态计算它们之间的相关性。每个位置的计算相互独立，只需要矩阵乘法
+ 为什么Transformer能够并行处理序列，而RNN必须串行处理？位置编码（Positional Encoding）在其中起什么作用？
  + RNN：每一步都依赖前一步，无法并行。Transformer：每个位置的计算相互独立，只需要矩阵乘法。位置编码：显式添加位置信息
+ Decoder-Only架构与完整的Encoder-Decoder架构有什么区别？为什么现在主流的大语言模型都采用Decoder-Only架构？
  + ![Decoder-Only与完整架构对比.png](../../images/Decoder-Only与完整架构对比.png)
  + 原因：统一的训练目标、更好的泛化能力、可以通过上下文学习新任务、简单，易于扩展到千亿参数

3、文本子词分词算法是大语言模型的一项关键技术，负责将文本转换为模型可处理的 token 序列。那为什么不能直接以"字符"或"单词"作为模型的输入单元？BPE（Byte Pair Encoding）算法解决了什么问题？
+ 答案：字符级分词：序列过长、语义丢失、计算量大、学习成本高，粒度太细。单词级分词：词汇表爆炸、OOV问题、形态变化、复合词、多语言，粒度太粗。
       BPE：在字符和单词之间找到平衡：用子词（subword）作为基本单元。
...
+ 调研前沿的研究和论文，是否还有其他的缓解模型幻觉的方法，他们又有哪些改进和优势？
  + ![缓解幻觉的方法.png](../../images/缓解幻觉的方法.png)
